...
export function FilterBar({
	...
	return (
		<div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-gray-900/50 p-4 rounded-xl border border-gray-800 backdrop-blur-sm shadow-xl">
			<div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
				<span className="text-sm font-medium text-gray-400 mr-2">交易所：</span>
				{exchanges.map((ex) => (
					<button
						type="button"
						key={ex.id}
              ...
              ${
					...
					>
						<span
							className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${ex.connected ? "bg-green-500 shadow-[0_0_4px_rgba(34,197,94,0.6)]" : "bg-gray-500"}`}
						/>
						{ex.label}
					</button>
				))}
